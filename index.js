...
const engineName = process.env.ENGINE_NAME !== undefined ? process.env.ENGINE_NAME : 'Rules Engine Template';
const createNewGame = require('./state/game').createNewGame;

function getRulesEnginePort() {
  const rawPortValue = process.env.RULES_ENGINE_PORT || '5005';
...
const createNewRulesEngine = function () {
  logger.info(`creating new rules engine`);
  const server = instantiateHttpServer();/**/
  const socket = io(server, {path: namespaceIdentifier});
